...
public class ChessBoard : MonoBehaviour
{
    ...
    [SerializeField] private float yOffset = 0.2f;   //the space between the board and pivot point
    ...
    [SerializeField] private GameObject[] prefabs; //array for prefabs
    [SerializeField] private Material[] teamMaterials; //array of materials
    ...
    //certain point in time where we start firing code 
    private void Start()
    {
        isWhiteTurn = true;
    ...
    private void Update()
    {
        ...
        {
            //Get indexes of the tile that user hit
            ...
            //If we already hovering a tile, change the previsou one
            ...
            if (Input.GetMouseButtonDown(0))
            {
                if (chessPieces[hitPosition.x, hitPosition.y] != null)
                {
                    ...
                    if ((chessPieces[hitPosition.x, hitPosition.y].team == 0 && isWhiteTurn && currentTeam == 0) || (chessPieces[hitPosition.x, hitPosition.y].team == 1 && !isWhiteTurn && currentTeam == 1))
                    {
                        ...
                        // Get a list of available moves, highlighting the tiles aswell
                        availableMoves = currentlyDragging.GetAvailableMoves(ref chessPieces, TILE_COUNT_X, TILE_COUNT_Y);

                        //Get a list of special moves aswell
            ...
            if (currentlyDragging != null && Input.GetMouseButtonUp(0))
            {
                ...
                if (ContainsVaildMove(ref availableMoves, new Vector2Int(hitPosition.x, hitPosition.y)))
                {
        ...
        //If we're dragging a piece (USEFUL!!!!!    )
    ...
    public void GameReset()
    {
        ...
        //Clean up dead pieces beside the board
    ...
    {
        //Save  the current values, to reset after the function all
        ...
        for (int i = 0; i < moves.Count; i++)
        {
            ...
            Vector2Int kingPoisitionThisSim = new Vector2Int(targetKing.currentX, targetKing.currentY);
            //Did we simulate the king's move
            if (cp.type == ChessPieceType.King)
                kingPoisitionThisSim = new Vector2Int(simX, simY);
            ...
            if (ContainsVaildMove(ref simMoves, kingPoisitionThisSim))
            {
    ...
    private bool CheckForCheckMate()
    {
        ...
        //Are we in check right now. (//TODO play sound when check mate)
        if (ContainsVaildMove(ref currentAvailableMoves, new Vector2Int(targetKing.currentX, targetKing.currentY)))
        {
            ...
            return true; //Check mate exit
        }

        return false;
    }
    //Operations
    private bool ContainsVaildMove(ref List<Vector2Int> moves, Vector2Int pos)
    {
        for (int i = 0; i < moves.Count; i++)
    ...
    private void MoveTo(int originalX, int originalY, int x, int y)
    {
        ...
        //if there is a piece in the target position
        if (chessPieces[x, y] != null)
        {
    ...
    private void OnWelcomeServer(NetMessage msg, NetworkConnection cnn)
    {
        ...
        // If two player connected , start the game
        if (playerCount == 1)
    ...
    {
        //We need to change the camera from UI to the game in here 
